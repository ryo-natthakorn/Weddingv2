...
- **Noted, not touched (pre-existing, out of scope):** the site's existing "By BTS/MRT" direction item (`direction_items` in `wedding-context.tsx`, both languages) says "take BTS to On Nut, then 10 min by taxi" — a different line/station entirely from the diorama's "Ram Inthra Kor Mor 6" (MRT Pink Line) marker. This predates the diorama work; flagged to Ryo to reconcile since it could confuse guests, but left as-is since it's unrelated existing content.
